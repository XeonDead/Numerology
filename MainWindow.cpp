...
void MainWindow::on_pushButton_clicked()
{
    ...
    auto lifeNumber  = Numerology::sumNumbers(dayNumber + monthNumber + std::get<0>(yearNumber));
    ...
    peaksChallenges.insert(1,
      { std::abs(dayNumber + monthNumber), std::abs(dayNumber - monthNumber) });
    peaksChallenges.insert(2,
      { std::abs(dayNumber + std::get<0>(yearNumber)), std::abs(dayNumber - std::get<0>(yearNumber)) });
    peaksChallenges.insert(3,
      { std::abs(peaksChallenges.value(1).first + peaksChallenges.value(2).first),
        std::abs(peaksChallenges.value(1).second - peaksChallenges.value(2).second) });
    peaksChallenges.insert(4,
      { std::abs(monthNumber + std::get<0>(yearNumber)), std::abs(monthNumber - std::get<0>(yearNumber)) });
    peaksChallenges.insert(5,
      { std::abs(peaksChallenges.value(3).first + peaksChallenges.value(4).first),
        std::abs(peaksChallenges.value(3).second - peaksChallenges.value(4).second) });

    QStringList strList;
    for (int i = 0; i < 5; ++i) {
        if (strList.empty()) {
            strList.push_back(QString("%1-%2").arg(Numerology::NumerologyNumber - std::get<0>(lifeNumber)).arg(Numerology::NumerologyNumber
              - std::get<0>(lifeNumber) + Numerology::NumerologyNumber / 3));
        } else {
    ...
    while (i.hasNext()) {
        ...
        if (std::get<2>(firstElemValue)) {
            ui_tableWidget->setItem(row, 0, new QTableWidgetItem(QString("(%1) %2").arg(std::get<1>(firstElemValue)).arg(std::get<0>(firstElemValue)), 0));
        } else {
            ui_tableWidget->setItem(row, 0, new QTableWidgetItem(QString("%1").arg(std::get<0>(firstElemValue)), 0));
        }

        auto secondElemValue = Numerology::sumNumbers(i.value().second);
        if (std::get<2>(secondElemValue)) {
            ui_tableWidget->setItem(row, 1, new QTableWidgetItem(QString("(%1) %2").arg(std::get<1>(secondElemValue)).arg(std::get<0>(secondElemValue)), 0));
        } else {
            ui_tableWidget->setItem(row, 1, new QTableWidgetItem(QString("%1").arg(std::get<0>(secondElemValue)), 0));
        }
